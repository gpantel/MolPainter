...
def assign_lattice_positions(path, solvent_lattice, solvent_molecule_id):
    solvu = mda.Universe(path)
...
def insert_to_random_empty_point2(path, solvent_lattice, solvent_molecule_id):
    ...
    # find +, - axis lengths of grid around solvent
    # positive x
    # @TODO this floor division is far too conservative -- molecules can nearly be on top of each other now
    # need to add some kind of "buffer" distance
...
#    if solvent_molecule_id == 1: print(xyz)
#    if solvent_molecule_id == 1: print(n_range_x, n_range_y, n_range_z)
...
for i in range(len(inputs['solvent_molecules']['paths'])):
    ...
    solvent_universe.atoms.positions = assign_lattice_positions(path, solvent_lattice, solvent_molecule_id)
    solvent_groups.append(solvent_universe.atoms)
    solvent_universe.atoms.write('solvent%i.pdb', bonds=None)

all_solvent_universe = mda.Merge(*solvent_groups)
all_solvent_universe.atoms.write('solvent_all.pdb', bonds=None)
